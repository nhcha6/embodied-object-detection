...
for split in ['train', 'val']:
    os.makedirs(os.path.join(output_root, f'temp_{split}'), exist_ok=True)
    k = 0
    ...
    for file in tqdm(files):
        ...
        if env in envs_splits['{}_envs'.format(split)]:
            ...
            # write the mask outliers and indices to file
            np.save(os.path.join(output_root, f'temp_{split}', f'{str(k)}_indices.npy'), indices)
            np.save(os.path.join(output_root, f'temp_{split}', f'{str(k)}_masks_outliers.npy'), mask_outliers)
