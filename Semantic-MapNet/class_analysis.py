...
mpcat40_classes = ['wall', 'chair', 'door', 'table', 'picture', 'cabinet', 'cushion', 'window', 'sofa', 'bed', 'curtain', 'chest_of_drawers', 'plant', 'sink', 'stairs', 'ceiling', 'toilet', 'stool', 'towel', 'mirror', 'tv_monitor', 'shower', 'column', 'bathtub', 'counter', 'fireplace', 'lighting', 'beam', 'railing', 'shelving', 'blinds', 'gym_equipment', 'seating', 'board_panel', 'furniture', 'appliances', 'clothes']

# chair, table, cushion, cabinet, shelving, sink, dresser, plant, bed, sofa, counter, fireplace
mapnet_classes = ['chair', 'table', 'cushion', 'cabinet', 'shelving', 'sink', 'dresser', 'plant', 'bed', 'sofa', 'counter', 'fireplace']

# read data
data=pd.read_csv('scannetv2-labels.combined.tsv',sep='\t')
# extract only the raw_category and mpcat40 columns
data=data[['raw_category','mpcat40']]
# iterate through each row in data
category_mapping = {}
for index, row in data.iterrows():
    # get the raw_category and mpcat40 values
    raw_category=row['raw_category']
    mpcat40=row['mpcat40']
    if mpcat40 in mpcat40_classes:
        category_mapping[raw_category] = mpcat40

# intersection of classes
lvis_mpcat40_classes = list(set(category_mapping.keys()).intersection(set(lvis)))
# only include classes that are in both datasets
category_mapping = {k: category_mapping[k] for k in lvis_mpcat40_classes}

# reverse the keys and values
reverse_category_mapping = {}
for k,v in category_mapping.items():
    if v in reverse_category_mapping.keys():
        reverse_category_mapping[v].append(k)
    else:
        reverse_category_mapping[v] = [k]

# print the reverse category mapping
for k, v in reverse_category_mapping.items():
    print(k, v)

# full dataset contains 20 classes with a mapping to LVIS
# 10 most common classes overlap with MapNet
# baseline object detectors can be acquired from DETIC paper: https://github.com/facebookresearch/Detic/blob/main/docs/MODEL_ZOO.md

# mpcat40 -> lvis one to one mapping
# bed bed
# stool stool
# towel towel
# fireplace fireplace
...
# sink sink
# seating bench
# tv_monitor television_set

# mpcat40 -> lvis one to many mapping
# bed ['headboard', 'mattress', 'bed']
# stool ['stool', 'ottoman', 'footstool']
# towel ['towel']
# fireplace ['fireplace']
