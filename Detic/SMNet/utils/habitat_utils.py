...
from .semantic_utils import use_fine, object_whitelist, object_lvis
from .semantic_utils import replica_to_mp3d_12cat_mapping, replica_to_lvis_mapping
...
class HabitatUtils:
    def __init__(self, scene, level, housetype='mp3d'):
        ...
        if housetype == 'mp3d':
            ...
            houses_dim = json.load(open('./houses_dim.json', 'r'))
        ...
        elif housetype == 'replica':
            self.all_objects = self.get_objects_replica()
            # self.all_objects = self.get_objects_in_house()
    ...
    def render_semantic_lvis_20(self):
        buf = self.sim.render(mode="semantic")
        out = np.zeros(buf.shape, dtype=np.uint8) # class 0 -> void
        object_ids = np.unique(buf)
        for oid in object_ids:
            ...
            if object_name in use_fine:
                object_name = object.category.name(mapping='raw')
            if object_name in object_lvis:
                object_index = object_lvis.index(object_name)+1
                out[buf==oid] = object_index
        return out
    
    def render_bbox_lvis_20(self):
        buf = self.sim.render(mode="semantic")
        out = []
        object_ids = np.unique(buf)
        for oid in object_ids:
            seg = np.where(buf==oid)
            x_min = int(np.min(seg[1]))
            x_max = int(np.max(seg[1]))
            y_min = int(np.min(seg[0]))
            y_max = int(np.max(seg[0]))

            # ignore if object edge is small
            if (x_max-x_min) < MIN_OBJECT_EDGE or (y_max-y_min) < MIN_OBJECT_EDGE:
                continue
            ...
            if object_name in use_fine:
                object_name = object.category.name(mapping='raw')
            if object_name in object_lvis:
                object_index = object_lvis.index(object_name)
                detection = {"category_id": object_index, "bbox": [x_min, y_min, x_max-x_min, y_max-y_min], "iscrowd": 0, "area": 0}
                out.append(detection)
        return out
    
    def render_semantic_lvis_replica(self):
        buf = self.sim.render(mode="semantic")
        out = np.zeros(buf.shape, dtype=np.uint8) # class 0 -> void
        object_ids = np.unique(buf)
        for oid in object_ids:
            if oid in self.all_objects:
                ...
                if object is not None:
                    object_name = object["class_name"]
                    if object_name in replica_to_lvis_mapping.keys():
                        object_name = replica_to_lvis_mapping[object_name]
                        object_index = object_lvis.index(object_name)+1
                        out[buf==oid] = object_index
        return out
    
    def render_bbox_lvis_replica(self):
        buf = self.sim.render(mode="semantic")
        out = []
        object_ids = np.unique(buf)
        for oid in object_ids:
    ...
    def get_objects_replica(self):
        semantic_path = self.scene[0:-8] + 'habitat/info_semantic.json'
        # open the json file
        with open(semantic_path, 'r') as f:
            semantic = json.load(f)
            id_to_label = semantic['id_to_label']
            objects = semantic['objects']
            objects = {object['id']: object for object in objects}
        return objects
