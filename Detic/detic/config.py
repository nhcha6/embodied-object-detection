...
def add_detic_config(cfg):
    ...
    # new training parameters for external memory
    _C.MODEL.MAP_MERGE_TYPE = ''
    _C.MODEL.MAP_FEAT_FUSION = ''
    _C.MODEL.FREEZE_BACKBONE = False
    _C.MODEL.UNFROZEN_LAYERS = []
    _C.DATASETS.SMNET_CFG = "SMNet/smnet.yml"
    _C.MODEL.MEMORY_FEATURE_WEIGHT = 100

    # semmap generation and usage
    _C.MODEL.TEST_SAVE_SEMMAP = False
    _C.MODEL.SEMMAP_PATH = ''
    _C.MODEL.MEMORY_TYPE = ''
    ...
    # MAMBA SETUP
    _C.MODEL.VID= CN()
    _C.MODEL.VID.MAMBA = CN()
    _C.MODEL.VID.MAMBA.SHUFFLE_TEST = True
    _C.MODEL.VID.MAMBA.INITIAL_FRAME_NUM = 25
    _C.MODEL.VID.MAMBA.REF_NUM = 2  
    _C.MODEL.MEMORY_BANK = CN()

    # Memory bank
    _C.MODEL.VID.MAMBA.MEMORY_BANK = CN()
    _C.MODEL.VID.MAMBA.MEMORY_BANK.MAX_NUM = 20000
    _C.MODEL.VID.MAMBA.MEMORY_BANK.KEY_NUM = 2000
    _C.MODEL.VID.MAMBA.MEMORY_BANK.SAMPLING_POLICY = "random"
    _C.MODEL.VID.MAMBA.MEMORY_BANK.UPDATING_POLICY = "random"

    # Pixel-level memory bank
    _C.MODEL.VID.MAMBA.PIX_MEM = CN()
    _C.MODEL.VID.MAMBA.PIX_MEM.ENABLE = True
    _C.MODEL.VID.MAMBA.PIX_MEM.NUM_ENHANCEMENT = 1
    _C.MODEL.VID.MAMBA.PIX_MEM.SCORE_THRESH = 0.2
    _C.MODEL.VID.MAMBA.PIX_MEM.STRIDE = 16
    _C.MODEL.VID.MAMBA.PIX_MEM.USE_RELU = False
    _C.MODEL.VID.MAMBA.PIX_MEM.SAMPLE_TRAIN = False

    _C.MODEL.VID.MAMBA.PIX_MEM.GEO = CN()
    _C.MODEL.VID.MAMBA.PIX_MEM.GEO.IN_CHANNELS = 256
    _C.MODEL.VID.MAMBA.PIX_MEM.GEO.EMB_CHANNELS = 256
    _C.MODEL.VID.MAMBA.PIX_MEM.GEO.NUM_HEADS = 16
    _C.MODEL.VID.MAMBA.PIX_MEM.GEO.OUTPUT_TYPE = "attention"
    _C.MODEL.VID.MAMBA.PIX_MEM.GEO.USE_EYE_INIT = False
    _C.MODEL.VID.MAMBA.PIX_MEM.GEO.QUERY_WEIGHT = 0.7

    # Instance-level memory bank
    _C.MODEL.VID.MAMBA.INS_MEM = CN()
    _C.MODEL.VID.MAMBA.INS_MEM.ENABLE = True
    _C.MODEL.VID.MAMBA.INS_MEM.NUM_ENHANCEMENT = 1
    _C.MODEL.VID.MAMBA.INS_MEM.REF_POST_NMS_TOP_N = 75

    _C.MODEL.VID.MAMBA.INS_MEM.GEO = CN()
    _C.MODEL.VID.MAMBA.INS_MEM.GEO.IN_CHANNELS = 1024
    _C.MODEL.VID.MAMBA.INS_MEM.GEO.EMB_CHANNELS = 1024
    _C.MODEL.VID.MAMBA.INS_MEM.GEO.NUM_HEADS = 16
    _C.MODEL.VID.MAMBA.INS_MEM.GEO.OUTPUT_TYPE = "attention"
    _C.MODEL.VID.MAMBA.INS_MEM.GEO.QUERY_WEIGHT = 0.7
