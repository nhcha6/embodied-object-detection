...
class Sequence():
    ...
    def print_sequence(self):
        ...
        for object in self.objects:
            print("Image id: " + str(object.image_id) + ", bbox: " + str(object.bbox) + ", score: " + str(object.score) + ", class_id: " + str(object.class_id))
    
class Object():
    def __init__(self, image_id, bbox, score, class_id):
...
def calculate_iou(box1, box2):
    ...
    Parameters:
    - box1: Tuple (x1, y1, x2, y2) representing the coordinates of the first rectangle.
    - box2: Tuple (x1, y1, x2, y2) representing the coordinates of the second rectangle.
...
if __name__ == "__main__":
    ...
    for folder in os.listdir(data_path):
        if os.path.isdir(data_path+folder):
            ...
            # create object recall variables
            current_sequences = []
            saved_sequences = []
            ...
            for image in images[::2]:
                ...
                # enumerate through pred_classes
                for i, pred_class in enumerate(pred_classes):
                    bbox = pred_boxes[i]
                    score = pred_scores[i]
                    for sequence in current_sequences:
                        if sequence.class_id == pred_class:
                            # check if iou > 0.7
                            if calculate_iou(sequence.current_object.bbox, bbox) > 0.7:
                                # lastly check that this sequence has not already been update
                                if sequence.current_object.image_id != img_count:
                                    # update the object
                                    sequence.add_object(Object(image_id=img_count, bbox=bbox, score=score, class_id=pred_class))
                            break
                    else:
                        # if it isn't, create a new sequence
                        current_sequences.append(Sequence(id=sequence_id, class_id=pred_class))
                        current_sequences[-1].add_object(Object(image_id=img_count, bbox=pred_boxes[i], score=pred_scores[i], class_id=pred_class))
                        sequence_id += 1

                # check if any sequences have ended
                keep_sequences = []
                for sequence in current_sequences:
                    if sequence.current_object.image_id == img_count:
                        keep_sequences.append(sequence)
                current_sequences = keep_sequences

                # save sequences over 5 image long
                for sequence in current_sequences:
                    if len(sequence.objects) > 4:
                        # delete current iteration of this sequence
                        for saved_seq in saved_sequences:
                            if sequence.id == saved_seq.id:
                                saved_sequences.remove(saved_seq)
                        # save sequence
                ...
                if img_count % 25 == 0:
                    ...
                    # for each class of interest - select optimal object and display results for assessment
